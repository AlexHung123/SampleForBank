postgres:
	docker run -d --name postgres12 -p 5432:5432 --network bank-network -e POSTGRES_USER=root -e POSTGRES_PASSWORD=secret postgres:12-alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/alexhung/simplebank/db/sqlc Store
